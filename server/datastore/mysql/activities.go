...
func buildPolicyAutomationBranches(ds *Datastore, policyID uint, filter fleet.TeamFilter, status string) ([]policyAutomationBranch, error) {
	...
	namedSQL, namedArgs, err := sqlx.In(fmt.Sprintf(`SELECT %s FROM activity_past ap %s
        WHERE ap.activity_type IN (?)
          AND ap.details->>'$.policy_id' = ?
          AND %s`, policyAutomationCols, policyAutomationHostJoin, teamFilterSQL), namedTypes, policyID)
	if err != nil {
		return nil, err
	}
	...
	for _, b := range policyAutomationTaskBranches {
		...
		sql := fmt.Sprintf("SELECT %s FROM activity_past ap %s %s WHERE ap.activity_type = ? AND %s",
			policyAutomationCols, policyAutomationHostJoin, b.joins, teamFilterSQL)
